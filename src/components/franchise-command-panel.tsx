"use client";
import {ReturnOutcomePanel} from "@/components/return-outcome-panel";
...
import { OperationsCollection } from "@/components/admin-ops/operations-list";
import { adminText, adminState } from "@/components/admin-ops/messages";
import adminStyles from "@/components/admin-ops/operations.module.css";
...
function OrderPaymentRequest({order,organization,provider,scope,disabled,compact=false}:{order:OrderOperations["orders"][number];organization:string;provider:string;scope:string;disabled:boolean;compact?:boolean}){
 const {locale:privateLocale,t,controlled}=usePrivateI18n();
  ...
  return <div className={compact?adminStyles.paymentAction:undefined}><button className="button" type="button" disabled={disabled||locked} onClick={()=>void requestPayment()}>{compact?(privateLocale.language==="en"?(provider==="fixture"?"Simulate payment request":"Request payment"):(provider==="fixture"?"Simular solicitud de pago":"Solicitar pago")):t("p0314")}</button>{compact?<p className={adminStyles.quiet}>{privateLocale.language==="en"?(provider==="fixture"?"Test mode · no real charge":"A request does not confirm payment"):(provider==="fixture"?"Modo de prueba · sin cobro real":"La solicitud no confirma el cobro")}</p>:<p>{t("p0315")} {provider}{t("p0316")}</p>}<p role="status" aria-label={`Resultado de pago ${order.id}`} aria-live="polite">{message}</p></div>;
}
...
export function OrderOperationsPanel({ snapshot, organization, canAllocate, canRequestPayment=false, paymentScope="" }: { snapshot: OrderOperations | null; organization: string; canAllocate: boolean;canRequestPayment?:boolean;paymentScope?:string }) {
 const {locale:privateLocale,t,controlled}=usePrivateI18n();
 const pathname=usePathname(),adminScope=pathname==="/franchise"||pathname==="/experience"||pathname?.startsWith("/experience/");
  ...
  const renderOrder=(order:OrderOperations["orders"][number])=>adminScope?<article className={adminStyles.orderBody} aria-label={`Pedido ${order.id}`}>
        <div className={adminStyles.orderLines}>{order.lines.map(line=><div key={line.id} className={adminStyles.orderLine}>
          <div>{order.lines.length>1?<strong>{line.name}</strong>:null}<span>{line.quantity} {privateLocale.language==="en"?(line.quantity===1?"unit":"units"):(line.quantity===1?"unidad":"unidades")}</span></div>
          {line.stock_id?<span className={adminStyles.quiet}>{privateLocale.language==="en"?"Unit reserved":"Unidad reservada"}</span>:<>
          {canAllocate&&["placed","confirmed"].includes(order.state)&&line.quantity===1?<form onSubmit={event=>void allocate(event,order,line)}>
            <label>{privateLocale.language==="en"?"Choose a unit":"Elegí una unidad"}<select name="stock" required defaultValue="" disabled={!ready||locked||snapshot!.truncated}><option value="" disabled>{t("p0350")}</option>{snapshot!.stock.filter(item=>item.variant_id===line.variant_id).map(item=><option key={item.id} value={item.id}>{item.serial}</option>)}</select></label>
        ...
        <dl className={adminStyles.orderFacts}>
          <div><dt>{privateLocale.language==="en"?"Payment":"Pago"}</dt><dd>{order.payments.length?order.payments.map(item=>state(item.state)).join(" · "):(privateLocale.language==="en"?"Not requested":"Sin solicitar")}</dd></div>
          <div><dt>{privateLocale.language==="en"?"Delivery":"Entrega"}</dt><dd>{order.handovers.length?order.handovers.map(item=>state(item.state)).join(" · "):(privateLocale.language==="en"?"Not prepared":"Por preparar")}</dd></div>
        </dl>
        {!snapshot!.payment_provider?<p>{t("p0357")}</p>:canRequestPayment&&paymentScope&&order.payments.length===0&&["placed","confirmed"].includes(order.state)?<OrderPaymentRequest compact order={order} organization={organization} provider={snapshot!.payment_provider} scope={paymentScope} disabled={!ready||locked||snapshot!.truncated}/>:null}
        <details className={adminStyles.relatedReferences}><summary>{privateLocale.language==="en"?"Related references":"Referencias relacionadas"}</summary><ul>{order.lines.filter(line=>line.stock_id).map(line=><li key={line.id}>{privateLocale.language==="en"?"Reserved unit":"Unidad reservada"} <code>{line.stock_id}</code></li>)}{order.payments.map(item=><li key={item.id}>{t("p0355")} <code>{item.id}</code></li>)}{order.handovers.map(item=><li key={item.id}>{t("p0358")} <code>{item.id}</code></li>)}</ul></details>
      </article>:<article className="card" key={order.id} aria-label={`Pedido ${order.id}`}>
        {!adminScope?<><h3>{t("p0251")} {order.id}</h3><p>{state(order.state)}</p></>:null}
        {order.lines.map(line=><div key={line.id}><h4>{line.name} {t("p0346")} {line.quantity}</h4>
          {line.stock_id ? <div><p>{t("p0347")}</p><details><summary>{privateLocale.language==="en"?"Unit reference":"Referencia de la unidad"}</summary><code>{line.stock_id}</code></details></div> : <><p>{t("p0348")}</p>
          {canAllocate && ["placed","confirmed"].includes(order.state) && line.quantity===1 ? <form onSubmit={event=>void allocate(event,order,line)}>
        ...
        <p>{t("p0355")} {order.payments.length ? order.payments.map(item=>`${state(item.state)}`).join("; ") : t("p0356")}</p>
        ...
        <p>{t("p0358")} {order.handovers.length ? order.handovers.map(item=>`${state(item.state)}`).join("; ") : t("p0359")}</p>
        {order.payments.length||order.handovers.length?<details><summary>{privateLocale.language==="en"?"Related references":"Referencias relacionadas"}</summary><ul>{order.payments.map(item=><li key={item.id}>{t("p0355")} <code>{item.id}</code></li>)}{order.handovers.map(item=><li key={item.id}>{t("p0358")} <code>{item.id}</code></li>)}</ul></details>:null}
      </article>;
  return <section className={adminScope?adminStyles.workspace:"card"} aria-labelledby="order-operations-title">
    <div className={adminScope?adminStyles.sectionHeading:undefined}>
    <h2 id="order-operations-title" className={adminScope?adminStyles.srOnly:undefined}>{t("p0339")}</h2>
    <button className={adminScope?adminStyles.secondary:"button"} type="button" disabled={!ready} onClick={()=>window.location.reload()}>{adminScope?adminText("refresh",privateLocale.language):t("p0341")}</button></div>
    {!adminScope?<p>{t("p0340")}</p>:null}
    <p className={adminScope?adminStyles.message:undefined} role="status" aria-label={t("p0342")} aria-live="polite">{message}</p>
    {!snapshot ? <p role="alert">{t("p0343")}</p> : <>
      {snapshot.truncated ? <p role="alert">{t("p0344")}</p> : null}
      {snapshot.orders.length===0 ? <p>{t("p0345")}</p> : adminScope?<OperationsCollection key={paymentScope||organization} items={snapshot.orders.map(order=>({id:order.id,title:order.lines.map(line=>line.name).join(" · ")||`${adminText("referenceOrder",privateLocale.language)} ${order.id}`,subtitle:`${adminText("referenceOrder",privateLocale.language)} · ${order.id.length>12?order.id.slice(-8):order.id}`,state:adminState(order.state,privateLocale.language)}))} language={privateLocale.language} label={adminText("orders",privateLocale.language)} testId="order" renderDetail={id=>renderOrder(snapshot.orders.find(order=>order.id===id)!)}/>:snapshot.orders.map(renderOrder)}
    </>}
    <OperationalGuide guide={OPERATIONAL_GUIDES["order-operations-view"]} {...(adminScope?{summaryLabel:"Ayuda"}:{})}/>
  </section>;
}
...
export function FranchiseCommandPanel({ permissions = [], leads, availability, exceptions, returnCases, defaultAssignee, organization, commandScope="" }: { permissions?: readonly string[]; leads: Lead[]; availability: Availability[]; exceptions: DeliveryException[] | null; returnCases: ReturnCase[] | null; defaultAssignee: string; organization: string;commandScope?:string }) {
 ...
 const pathname=usePathname(),adminScope=pathname==="/franchise"||pathname==="/experience"||pathname?.startsWith("/experience/");
  ...
  const contactTitle=(lead:Lead)=>lead.created_at&&Number.isFinite(Date.parse(lead.created_at))?experienceText("Contacto del ",privateLocale.language)+new Date(lead.created_at).toLocaleDateString(privateLocale.locale,{day:"numeric",month:"short",timeZone:privateLocale.timeZone}):experienceText("Contacto",privateLocale.language);
  const contactTasks=(lead:Lead)=><TaskWorkspace title="Trabajo con el contacto"><section data-task-label="Seguimiento"><LeadActions lead={lead} scope={commandScope} defaultAssignee={defaultAssignee} canAssign={can("lead:assign")} canTransition={can("lead:update")}/></section>{can("quote:write") ? <section data-task-label="Cotización"><QuoteCreation lead={lead} scope={commandScope}/></section> : null}</TaskWorkspace>;
  const renderContact=(lead:Lead)=><article className="card" key={lead.id}>
      {!adminScope?<><h2>{contactTitle(lead)}</h2><p>{experienceText(({new:"Nuevo",contacted:"Contactado",qualified:"Calificado",converted:"Convertido",lost:"No concretado",closed:"Cerrado"} as Record<string,string>)[lead.state]??lead.state,privateLocale.language)} · {experienceText(lead.source_code==="website"?"Sitio web":lead.source_code,privateLocale.language)}</p><details className="referenceDetails"><summary>{experienceText("Referencia del contacto",privateLocale.language)}</summary><code>{lead.id}</code></details></>:null}
      <p>{lead.assigned_subject ? `${privateLocale.language==="en"?"Assignee":"Responsable"}: ${lead.assigned_subject}` : t("p0378")}</p>
      {adminScope?contactTasks(lead):<details className="contactWork" name="franchise-contact"><summary>{experienceText("Gestionar contacto",privateLocale.language)}</summary>{contactTasks(lead)}</details>}
    </article>;
  return <TaskWorkspace title="Trabajo de gestión" initialLabel="Contactos">
    ...
    <div data-task-label="Contactos" className={adminScope?adminStyles.workspace:"grid"}>{adminScope?<OperationsCollection key={commandScope} items={leads.map(lead=>({id:lead.id,title:contactTitle(lead),subtitle:experienceText(lead.source_code==="website"?"Sitio web":lead.source_code,privateLocale.language),state:adminState(lead.state,privateLocale.language)}))} language={privateLocale.language} label={adminText("contacts",privateLocale.language)} testId="contact" renderDetail={id=>renderContact(leads.find(lead=>lead.id===id)!)}/>:leads.map(renderContact)}</div>
  </TaskWorkspace>;
}
...
export function ChecklistCompletionPanel({organization,scope,initialHandover,recoveryOnly=false}:{organization:string;scope:string;initialHandover?:{id:string;version:number};recoveryOnly?:boolean}){
 const {locale:privateLocale,t,controlled}=usePrivateI18n();
 const pathname=usePathname(),adminScope=pathname==="/franchise"||pathname==="/experience"||pathname?.startsWith("/experience/");
 ...
 useEffect(()=>{try{
  ...
  if(raw!==null){const v=JSON.parse(raw);if(!v||Object.keys(v).sort().join(",")!=="checklistId,checklistVersion,digest,handoverId,version"||!id(v.handoverId)||(initialHandover&&v.handoverId!==initialHandover.id)||!version(v.version)||!code(v.checklistId)||!version(v.checklistVersion)||!/^[a-f0-9]{64}$/.test(v.digest))throw new Error("reference");operation.current=v;setLocked(true);setMessage(t("p0547"))}
  else{operation.current=null;setLocked(false)}setReady(true);
 }catch{setReady(true);setLocked(true);setMessage(t("p0518"))}},[scope,storageKey]);
 async function submit(event:FormEvent<HTMLFormElement>){
   event.preventDefault();if(recoveryOnly||!ready||locked||fence.current||!guidedReady)return;const data=new FormData(event.currentTarget);
 ...
 function prepare(){if(recoveryOnly||!confirmed||posting||reading.current)return;try{sessionStorage.removeItem(storageKey);operation.current=null;fence.current=false;setLocked(false);setConfirmed(false);setResult(null);form.current?.reset();setMessage(t("p0554"))}catch{setMessage(t("p0527"))}}
 // A committed checklist can outlive its prepared state. Retain its original marker
 // and expose only read recovery after that transition; never manufacture a new send.
 if(recoveryOnly&&!operation.current&&!message)return null;
 return <div className={adminScope?adminStyles.checklist:undefined} role="region" aria-label={t("p0555")} data-testid="checklist-completion" data-recovery-only={recoveryOnly}>{!recoveryOnly?<form ref={form} onSubmit={event=>void submit(event)}><fieldset disabled={!ready||locked}><legend>{adminScope?adminText("checklist",privateLocale.language):t("p0556")}</legend><GuidedChecklistFields organization={organization} handover={initialHandover} onReady={setGuidedReady}/><div className={adminScope?adminStyles.formActions:undefined}><button className={adminScope?"button":undefined} disabled={!ready||locked||!initialHandover||!guidedReady}>{experienceText("Confirmar revisión",privateLocale.language)}</button></div></fieldset></form>:null}
 <p className={adminScope?adminStyles.message:undefined} role="status" aria-label={t("p0562")} aria-live="polite">{message}</p>
 ...
 {confirmed&&!recoveryOnly?<button type="button" disabled={posting||querying} onClick={prepare}>{t("p0567")}</button>:null}
