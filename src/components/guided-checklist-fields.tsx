...
import { adminText } from "@/components/admin-ops/messages";
...
export function GuidedChecklistFields({ organization, handover, onReady }: {
    ...
    return <div className={adminScope ? styles.fields : undefined} data-testid="guided-checklist-fields">
 ...
 <div className={adminScope ? styles.fieldItems : "stack"} key={`${checklist.id}:${checklist.version}`}>{checklist.items.map(item => item.response_type === "confirmation" ? <label className={adminScope ? styles.checkRow : "checkRow"} key={item.id}>
