...
$libraryRoot = [IO.Path]::GetFullPath($LibraryRoot).TrimEnd([IO.Path]::DirectorySeparatorChar)
if ((Get-FileHash -LiteralPath (Join-Path $libraryRoot "VERIFY_LIBRARY.ps1") -Algorithm SHA256).Hash.ToLowerInvariant() -cne "99c6a0874aa0324416f05d4318372c293109fb2538b31003fbde05170319680d") { throw "Historical verifier differs from pinned predecessor" }
...
function Find-MachineLocalHomePath([string] $Text) {
  ...
  if ($profile -and $Text.Contains($profile, [StringComparison]::OrdinalIgnoreCase)) {
    return [regex]::Match($Text, [regex]::Escape($profile), [Text.RegularExpressions.RegexOptions]::IgnoreCase)
  }
  $user = [Environment]::UserName
  $pattern = '(?:(?i:[A-Z]:\\Users\\[^\\\s`"''<>]+)|(?<![A-Za-z0-9:])/(?:home|Users)/[^/\s`"''<>]+)'
  foreach ($hit in [regex]::Matches($Text, $pattern)) {
    if ($hit.Value -match '^[A-Za-z]:\\') { return $hit }
    $name = ($hit.Value -split '/')[-1]
    if ($name -ceq $user) { return $hit }
  }
...
function Assert-AgentEntry($item) {
  Assert-True ($item.Name -ceq '.agents') 'agents directory casing differs'
  Assert-True (($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0) 'agents root is a link'
  $expected = @('skills','skills/elite-engineering-library','skills/elite-engineering-library/SKILL.md')
  $entries = @(Get-ChildItem -LiteralPath $item.FullName -Recurse -Force)
  Assert-True ($entries.Count -eq 3) 'agents tree must contain exactly two directories and one skill'
  foreach ($entry in $entries) {
    $rel = [IO.Path]::GetRelativePath($item.FullName,$entry.FullName).Replace('\','/')
    Assert-True ($expected -ccontains $rel) 'unexpected agents child or casing'
    Assert-True (($entry.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0) 'agents child is a link'
  }
  $skill = Join-Path $item.FullName 'skills/elite-engineering-library/SKILL.md'
  Assert-True (Test-Path -LiteralPath $skill -PathType Leaf) 'agents skill missing'
  Assert-True ((Get-FileHash -LiteralPath $skill -Algorithm SHA256).Hash.ToLowerInvariant() -ceq '9ce0a40fb895f2597250602c23b79284da7312517fbde68318c6012128f28216') 'agents skill pointer drift'
}
...
function Get-ReleaseSourceFiles {
  ...
  foreach ($item in Get-ChildItem -LiteralPath $libraryRoot -Force) {
    ...
    if ($item.PSIsContainer) {
      if ($item.Name -ieq '.agents') { Assert-AgentEntry $item }
...
try {
  ...
  $historicalCounts = @{ AUTHORED = 0; ADAPTED = 0; VERBATIM = 0 }
  $successorCounts = @{ AUTHORED = 0; ADAPTED = 0; VERBATIM = 0 }
  foreach ($packFile in $packFiles) {
    ...
    foreach ($provenance in @(Read-MaterializationProvenance $packFile.FullName)) {
      ...
      if ($metadata.Id -ceq "UNIFIED-REFERENCE-V403-R4") { $successorCounts[$provenance]++ } else { $historicalCounts[$provenance]++ }
    }
  }
  ...
  $historicalTotal = $historicalCounts.AUTHORED + $historicalCounts.ADAPTED + $historicalCounts.VERBATIM
  $expectedProvenanceLine = "Current-Provenance-Counts: AUTHORED=$($historicalCounts.AUTHORED); ADAPTED=$($historicalCounts.ADAPTED); VERBATIM=$($historicalCounts.VERBATIM); TOTAL=$historicalTotal"
  ...
  $successorNotices = [IO.File]::ReadAllText((Join-Path $libraryRoot 'markdown_system/THIRD_PARTY_NOTICES_V403.md'), $utf8)
  $successorTotal = $successorCounts.AUTHORED + $successorCounts.ADAPTED + $successorCounts.VERBATIM
  $deltaLine = "Delta-Provenance-Counts: AUTHORED=$($successorCounts.AUTHORED); ADAPTED=$($successorCounts.ADAPTED); VERBATIM=$($successorCounts.VERBATIM); TOTAL=$successorTotal"
  $combinedLine = "Current-Provenance-Counts: AUTHORED=$($provenanceCounts.AUTHORED); ADAPTED=$($provenanceCounts.ADAPTED); VERBATIM=$($provenanceCounts.VERBATIM); TOTAL=$provenanceTotal"
  Assert-True ($successorNotices.Contains($deltaLine, [StringComparison]::Ordinal)) "successor provenance delta differs: $deltaLine"
  Assert-True ($successorNotices.Contains($combinedLine, [StringComparison]::Ordinal)) "successor provenance total differs: $combinedLine"

  foreach ($planFile in Get-ChildItem -LiteralPath (Join-Path $libraryRoot 'markdown_system') -File | Where-Object { $_.Name -like '*PACK_PLAN.md' -or $_.Name -ceq 'UNIFIED_REFERENCE_PACK_PLAN_V403_R4.md' } | Sort-Object Name) {
  ...
  $profile = 'UNIFIED_REFERENCE_PACK_PLAN_V403_R4.md'
  $destination = Join-Path $tempRoot ([IO.Path]::GetFileNameWithoutExtension($profile))
  & $compositor -PlanFile (Join-Path $libraryRoot "markdown_system/$profile") -LibraryRoot $libraryRoot -Destination $destination | Out-Null
  Assert-True (Test-Path -LiteralPath (Join-Path $destination 'MATERIALIZATION_RECORD.md')) "composition record missing for $profile"
  $profileFiles = (Get-ChildItem -LiteralPath $destination -Recurse -File | Measure-Object).Count - 1
  Assert-True ($profileFiles -eq 2113) "successor carrier count differs: expected=2113 actual=$profileFiles"
  $profileResults.Add([pscustomobject]@{ Profile = $profile; Files = $profileFiles })
  ...
  $allPlanNames = @(Get-ChildItem -LiteralPath (Join-Path $libraryRoot 'markdown_system') -File | Where-Object { $_.Name -like '*PACK_PLAN.md' -or $_.Name -ceq 'UNIFIED_REFERENCE_PACK_PLAN_V403_R4.md' } | ForEach-Object Name | Sort-Object)
  ...
  $preflightSummary = Get-Content -LiteralPath (Join-Path $libraryRoot 'markdown_system/V403_SUCCESSOR_PREFLIGHT.md') -Raw
  ...
  $expectedFranchise = "El perfil histórico V402 selecciona $($franchiseProfile[0].Files) archivos"
  ...
  Write-Output 'VERIFY_LIBRARY_V403_PASS'
